...
GrandeBook::GrandeBook(string name, const int grandeArray[][tests])
{
    setCourseName(name);
...
double GrandeBook::getAverage(const int setOfGrandes[], const int grandes)
{
    ...
    {
        totale += setOfGrandes[i];
    }

    return static_cast<double>(totale) / grandes;
}
...
void GrandeBook::outputGrandes()
{
    ...
    for (int i = 0; i < students; i++)
    {
        ...
        for (int j = 0; j < tests; j++)
        {
            cout << setw(8) << voti[i][j];
        }

        double average = getAverage(voti[i], tests);
